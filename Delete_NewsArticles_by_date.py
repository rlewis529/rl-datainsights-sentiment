...
def main():
    ...
    if choice in {"1", "2"}:
        ...
        print("\n⚠️ This will DELETE ALL records in the NewsArticles table!")
    ...
    if choice == "3":
        cursor.execute("DELETE FROM NewsArticles")
        cursor.execute("DELETE FROM sqlite_sequence WHERE name='NewsArticles'")
    else:
        cursor.execute(f"""
    ...
    print(f"✅ Deleted {deleted} article(s) from NewsArticles.")
